...
func _process(delta):
	...
	if player:
		goal = player.get_global_transform().origin+Vector3.UP*4
	var senses = get_parent().get_node("Senses")
	var position = self.get_global_transform().origin
	var trans = self.get_parent().get_global_transform().looking_at(goal,Vector3.UP)
	var next_trans = self.get_parent_spatial().get_global_transform().interpolate_with(trans,delta*rng.randf_range(-4,6))
	self.get_parent_spatial().set_global_transform(next_trans)
	
	#Bat does not fly up or down, not sure.
	var move_vec =Vector3(0,-next_trans.basis.z.length(),-next_trans.basis.z.length())
